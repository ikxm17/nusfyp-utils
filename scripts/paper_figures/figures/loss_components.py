...
def plot(experiment, output_dir, smooth_window=100, formats=("pdf", "png"),
         width="single", no_phase=False, budget=False, **kwargs):
    ...
    if budget and total_series is not None:
        _plot_budget(ax, loss_data, total_series, smooth_window)
        ax.set_ylabel("Fraction of Total Loss")
        ax.set_ylim(0, 1.05)
    else:
        _plot_absolute(ax, loss_data, smooth_window)
        ax.set_ylabel("Loss")
        ax.set_yscale("log")
...
def _plot_absolute(ax, loss_data, smooth_window):
